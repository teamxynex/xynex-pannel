...
get_panel_status_line() {
    ...
    echo -e "${BLUE}Panel:${NC} ${status_color}${running}${NC} ${BLUE}·${NC} ${BLUE}Port:${NC} ${port} ${BLUE}·${NC} ${BLUE}URL:${NC} http://${ip:-<server-ip>}:${port}"
}
...
print_banner() {
    ...
    echo "  ╚════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
    echo -e "  ${BLUE}v${INSTALLER_VERSION} · https://github.com/teamxynex/xynex-pannel${NC}"
...
install_panel() {
    ...
    if command -v apt-get &> /dev/null; then
        run_with_progress "Updating & installing base packages" \
            'sudo apt-get update -y; sudo apt-get install -y curl git build-essential ca-certificates unrar p7zip-full; command -v unrar &> /dev/null || sudo apt-get install -y unrar-free'
    elif command -v yum &> /dev/null; then
        run_with_progress "Updating & installing base packages" \
            'sudo yum update -y; sudo yum install -y curl git make gcc-c++ ca-certificates unrar p7zip'
    fi

    # Setup and install Node.js 20.x if not already at least v20
    if ! command -v node &> /dev/null || [ $(node -v | cut -d'.' -f1 | tr -d 'v') -lt 20 ]; then
        run_with_progress "Installing Node.js 20.x" \
    ...
    if ! command -v pm2 &> /dev/null; then
        run_with_progress "Installing PM2" \
            'sudo npm install -g pm2; npm install pm2 -D'
    else
    ...
    if [ "$ENGINE_CHOICE" == "2" ]; then
        ...
        if ! command -v podman &> /dev/null; then
            if command -v apt-get &> /dev/null; then
                run_with_progress "Installing Podman" 'sudo apt-get install -y podman podman-docker'
            elif command -v yum &> /dev/null; then
                run_with_progress "Installing Podman" 'sudo yum install -y podman podman-docker'
            fi
        fi
        if command -v podman &> /dev/null; then
        ...
        if ! command -v docker &> /dev/null; then
            run_with_progress "Installing Docker" \
                'curl -fsSL https://get.docker.com | sh; command -v systemctl &> /dev/null && sudo systemctl enable --now docker'
        else
        ...
        if ! command -v cloudflared &> /dev/null; then
            ...
            if command -v apt-get &> /dev/null; then
                run_with_progress "Installing cloudflared" \
                    "curl -fsSL -o /tmp/cloudflared.deb 'https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-${CF_ARCH}.deb'; sudo dpkg -i /tmp/cloudflared.deb || sudo apt-get install -f -y"
            elif command -v yum &> /dev/null; then
                run_with_progress "Installing cloudflared" \
                    "curl -fsSL -o /tmp/cloudflared.rpm 'https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-${CF_ARCH}.rpm'; sudo yum install -y /tmp/cloudflared.rpm"
            fi
        fi
...
EOF
    ...
    run_with_progress "Installing Node.js dependencies" "npm i"

    run_with_progress "Building panel" "npm run build"
    ...
    run_with_progress "Starting panel with PM2" "npx pm2 start ecosystem.config.cjs && npx pm2 save"
...
update_panel() {
    ...
    run_with_progress "Installing dependencies" "npm i"

    run_with_progress "Rebuilding panel" "npm run build"

    run_with_progress "Restarting PM2 process" "npx pm2 restart xynex-panel || npx pm2 restart all"
...
case "$ACTION" in
    ...
    menu)
        while true; do
            ...
            echo -e "  ${BLUE}│${NC}  ${BOLD}3)${NC} Update Panel                                     ${BLUE}│${NC}"
            echo -e "  ${BLUE}│${NC}  ${BOLD}4)${NC} Create Admin User                                ${BLUE}│${NC}"
            echo -e "  ${BLUE}│${NC}  ${BOLD}5)${NC} Restart Panel                                    ${BLUE}│${NC}"
            echo -e "  ${BLUE}│${NC}  ${BOLD}6)${NC} Show Install Logs                                ${BLUE}│${NC}"
            echo -e "  ${BLUE}│${NC}  ${BOLD}7)${NC} Exit                                             ${BLUE}│${NC}"
            echo -e "  ${BLUE}└──────────────────────────────────────────────────────┘${NC}"
            ...
            case "$CHOICE" in
                ...
                7)
                    ...
                    ;;
                *)
                    log_error "Invalid option! Please enter a number from 1 to 7."
                    sleep 1.5
                    ;;
            esac
        done
