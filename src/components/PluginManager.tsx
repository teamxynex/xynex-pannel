...
export default function PluginManager({ serverId }: { serverId: string }) {
  ...
  const handleInstall = async (plugin: Plugin) => {
    ...
    try {
      ...
      notify(res.data.message || `${plugin.name} installed successfully! Restart the server to apply changes.`);
    } catch (e: any) {
      notify(e.response?.data?.error || "Failed to install plugin.");
    } finally {
  ...
  const getSourceIcon = (source: string) => {
    switch (source) {
      ...
      default: return <Puzzle className="w-3 h-3 text-theme-400" />;
    }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          <div>
            ...
            <p className="text-[11px] font-bold text-theme-400/80 uppercase tracking-widest mt-1">Search and install plugins from Modrinth, Spigot, and Paper Hangar.</p>
          </div>
        </div>
        ...
        <div className="bg-black/40 dark:bg-black/40 backdrop-blur-xl border border-border rounded-3xl overflow-hidden shadow-[0_0_40px_-15px_rgba(0,0,0,0.5)] ring-1 ring-border-subtle">
          <div className="p-4 border-b border-border-subtle space-y-4">
            <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-2">
              ...
              <button 
                type="submit"
                className="px-4 py-2 bg-theme-500 hover:bg-theme-600 text-foreground rounded-lg text-sm font-medium transition-colors whitespace-nowrap shrink-0"
              >
                Search
              </button>
            </form>
            ...
            <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-1">
              {['all', 'modrinth', 'spigot', 'hangar'].map(src => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors whitespace-nowrap flex items-center gap-1.5 ${activeSource === src ? 'bg-theme-500 text-foreground' : 'bg-muted text-muted-foreground hover:bg-muted-hover hover:text-foreground'}`}
                >
          ...
          <div className="divide-y divide-border-subtle">
            {loading ? (
              <div className="p-8 text-center text-muted-foreground flex flex-col items-center">
                <RefreshCw className="w-6 h-6 animate-spin mb-3 text-theme-500/50" />
              ...
              plugins.map((plugin) => (
                <div key={`${plugin.source}-${plugin.id}`} className="p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:bg-muted-subtle transition-colors">
                  ...
                  <button
                    ...
                    className="w-full md:w-auto px-4 py-2 bg-muted hover:bg-theme-500/10 border border-border hover:border-theme-500/30 text-foreground-muted hover:text-theme-400 rounded-lg text-sm font-medium transition-all flex items-center justify-center shrink-0 disabled:opacity-50"
                  >
