...
// Network throughput doesn't have a fixed cap like CPU/RAM/disk, so the
// sparkline's max scales to whatever the highest recent sample was (with a
// sane floor so it doesn't look maxed-out at a few KB/s).
const NET_FLOOR_KBPS = 64;
...
export default function ServerConsole({ serverId, server }: ServerConsoleProps) {
  ...
  const [eulaNeeded, setEulaNeeded] = useState(false);
  ...
  // Pterodactyl-style EULA prompt: vanilla/Paper/Spigot/Forge images refuse
  // to start until eula.txt has eula=true. Poll for it (and react instantly
  // if the server's own log says so) instead of making the user dig into
  // the file manager to find and edit it themselves.
  useEffect(() => {
    if (!isMinecraft || !serverId) return;
    let cancelled = false;
    const check = () => {
      axios.get(`/api/servers/${serverId}/eula`).then((res) => {
        if (!cancelled) setEulaNeeded(res.data.exists && !res.data.accepted);
      }).catch(() => {});
    };
    check();
  ...
  useEffect(() => {
    ...
    socket.on("log", (data: string) => {
      ...
      if (isMinecraft && /you need to agree to the eula/i.test(data)) {
        setEulaNeeded(true);
      }
      ...
      try {
        ...
        if (alive && data) {
          setStats((p) => ({
            ...
            netIn: data.netIn ?? p.netIn,
            netOut: data.netOut ?? p.netOut,
          ...
          setNetInHist((h) => [...h, data.netIn ?? 0].slice(-SPARK_CAP));
          setNetOutHist((h) => [...h, data.netOut ?? 0].slice(-SPARK_CAP));
        }
  ...
  const renderLine = useCallback((raw: string): React.ReactNode => {
    ...
    return (
      <span className={`flex-1 flex items-stretch min-w-0`}>
        ...
        <span className={`break-words whitespace-pre-wrap min-w-0 text-[11px] sm:text-xs xl:text-[13px] leading-[1.6] xl:leading-[1.75] ${text}`}>
  ...
  // Network has no fixed cap, so scale sparklines/dial to the busiest recent
  // sample (with a floor so idle traffic doesn't look like a maxed-out bar).
  const netMax = useMemo(
    () => Math.max(NET_FLOOR_KBPS, ...netInHist, ...netOutHist, stats.netIn, stats.netOut),
    [netInHist, netOutHist, stats.netIn, stats.netOut]
  );
  ...
  const renderTelemetryPanel = () => (
    <section className="qx-panel rounded-[24px] relative overflow-hidden">
      ...
      <div className="qx-telemetry-row flex items-center justify-between gap-3 px-3 sm:px-4 py-3">
        <div className="flex items-center gap-3 min-w-0">
          <Dial
            pct={Math.min(((stats.netIn + stats.netOut) / (netMax * 2)) * 100, 100)}
            color="#38bdf8"
          ...
          <div className="min-w-0">
            ...
            <p className="qx-mono text-lg sm:text-[22px] font-bold leading-none text-sky-300 flex items-center gap-1">
              <ArrowDownToLine size={12} className="text-sky-400/70" />
              <AnimNum value={stats.netIn} />
              <span className="text-[11px] text-sky-300/50 ml-0.5">KB/s</span>
            </p>
            <p className="qx-mono text-[11px] text-slate-400 mt-1 flex items-center gap-1">
              <ArrowUpFromLine size={11} className="text-sky-500/60" />
              <AnimNum value={stats.netOut} />
              <span className="text-[10px] text-slate-500 ml-0.5">KB/s out</span>
            </p>
          </div>
        </div>
        <div className="shrink-0 xs:flex flex-col gap-1.5 items-end">
          <Spark data={netInHist} color="#38bdf8" max={netMax} w={90} h={16} />
          <Spark data={netOutHist} color="#0ea5e9" max={netMax} w={90} h={16} />
        </div>
      </div>
    </section>
  );
  ...
  const handleAcceptEula = async () => {
    ...
    try {
      await axios.post(`/api/servers/${serverId}/eula/accept`);
      setEulaNeeded(false);
      setLogs((p) => [...p, "[System] Minecraft EULA accepted. Start the server again to apply it."]);
    } catch {
      setLogs((p) => [...p, "[System] Failed to accept the EULA. Please try again."]);
    } finally {
  ...
  return (
    <>
      ...
      <div className="absolute inset-0 overflow-y-auto text-foreground touch-auto overscroll-y-auto qx-scroll bg-transparent">
        <div className="relative flex flex-col xl:flex-row w-full max-w-[1800px] mx-auto min-h-full gap-3 md:gap-5 p-3 md:p-6 pb-20 md:pb-10">
          
          {/* ═══════════ MINECRAFT EULA PROMPT ═══════════ */}
          {isMinecraft && eulaNeeded && (
            <div className="w-full order-first flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3.5">
              <div className="flex items-start gap-3">
                <AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-amber-300">You need to accept the Minecraft EULA</p>
                  <p className="text-xs text-amber-200/70 mt-0.5">
                    This server can't start until you agree to Mojang's End User License Agreement.{" "}
                    <a href="https://aka.ms/MinecraftEULA" target="_blank" rel="noreferrer" className="underline hover:text-amber-100">Read the EULA</a>
                  </p>
                </div>
              </div>
              <button
                ...
                className="flex-shrink-0 px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 disabled:opacity-60 text-black font-bold text-sm transition-colors whitespace-nowrap"
              >
                {eulaAccepting ? "Accepting…" : "I Agree — Accept EULA"}
              </button>
            </div>
          )}
          ...
          {/* ═══════════ DESKTOP RIGHT SIDEBAR — TELEMETRY + PLAYERS ═══════════ */}
          <aside
            className={`hidden xl:flex flex-col gap-5 xl:w-[380px] shrink-0 order-2 xl:order-2 ${
          ...
          <div className={`flex-1 flex-col gap-4 order-1 xl:order-1 ${mobileTab === "console" ? "flex" : "hidden xl:flex"}`}>
            <section
            ...
            >
              {/* ── Header ── */}
              <header className="px-3 md:px-5 xl:px-7 py-2.5 sm:py-3 flex items-center justify-between gap-2 border-b border-border-subtle relative z-10">
              ...
              <div
                ref={bodyRef}
                onScroll={onScroll}
                className="flex-1 overflow-y-auto px-2.5 sm:px-4 md:px-6 xl:px-7 py-3 sm:py-4 qx-mono text-[11px] md:text-xs xl:text-[13px] leading-[1.7] qx-scroll relative z-10"
              ...
              <form
                onSubmit={send}
                className="p-2 sm:p-3 md:p-4 xl:p-5 flex gap-2 relative z-10 bg-black/40 backdrop-blur-md border-t border-border-subtle"
              >
