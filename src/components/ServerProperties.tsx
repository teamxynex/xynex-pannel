...
export default function ServerProperties({ serverId }: { serverId: string }) {
  ...
  const handleSave = async () => {
    try {
      ...
      notify('Properties saved successfully! You may need to restart the server for changes to take effect.');
    } catch (e) {
      notify('Failed to save properties.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <RefreshCw className="w-5 h-5 text-theme-500 animate-spin" />
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-theme-400/80 font-bold uppercase tracking-widest text-xs mt-1">Configure core server rules and settings</p>
          </div>
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="px-5 py-2.5 bg-theme-600 hover:bg-theme-500 text-foreground font-bold rounded-xl transition-all flex items-center disabled:opacity-50 shadow-[0_0_15px_rgba(79,70,229,0.3)] hover:shadow-[0_0_20px_rgba(99,102,241,0.5)] ring-1 ring-border"
          >
            ...
            <div key={prop.key} className="bg-black/40 dark:bg-black/40 backdrop-blur-xl border border-border p-5 rounded-2xl shadow-[0_0_30px_-15px_rgba(0,0,0,0.5)] ring-1 ring-border-subtle relative overflow-hidden group hover:bg-black/60 transition-colors">
                ...
                {prop.type === 'boolean' && (
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${
                        properties[prop.key] === 'true' ? 'bg-theme-500' : 'bg-zinc-700'
                    }`}
                  >
                    <span
              ...
              {prop.type === 'select' && (
                <select
                  ...
                  className="w-full bg-black/60 border border-border focus:border-theme-500 rounded-xl px-4 py-2.5 text-sm font-medium text-foreground outline-none shadow-inner transition-colors"
                >
              ...
              {Object.keys(properties).filter(k => !COMMON_PROPERTIES.find(cp => cp.key === k)).map(key => (
                <div key={key} className="flex flex-col">
                  ...
                  <input
                    ...
                    className="w-full bg-card border border-border focus:border-theme-500 rounded-lg px-3 py-1.5 text-sm text-foreground outline-none font-mono"
                  />
                </div>
              ))}
            </div>
